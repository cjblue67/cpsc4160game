...
void Hud::draw(int seconds, int FPS, int lives, int enemies) const {
    ...
    io.printMessageValueAt("Lives Remaining: ", lives, 10, 220);
    io.printMessageValueAt("Enemies destroyed: ", enemies, 10, 250);
    io.printMessageAt("Kill 50 enemies to escape with", 10, 280);
    io.printMessageAt("the plans and save the galaxy!", 10, 310);
}
